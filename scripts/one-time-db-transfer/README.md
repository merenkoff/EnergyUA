...
Видали з `package.json` скрипти `db:ot:transfer`, `db:ot:dump` та `db:ot:restore`, переконайся що **`db:predeploy`** лише `prisma migrate deploy && prisma db seed`, з `.gitignore` — секцію `One-time DB transfer`. Закоміть — другий етап завершено.
