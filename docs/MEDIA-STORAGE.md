...
1. **Найшвидше, якщо локально вже є той самий `storage/media`** (ті самі `sha256.ext`, що в БД): з Mac виконай передачу на volume через SSH-потік:
   ...
   ```
   (за замовчуванням сервіс `EnergyUA`; інший: `npm run db:push-media-railway -- ІмяСервісу`. Каталог на сервері: `RAILWAY_REMOTE_MEDIA_ROOT=/data/media`.) Потім знову `bash /app/scripts/railway-media-diagnose.sh`. Gzip-потік пишеться у **тимчасовий файл** на контейнері через **`dd`** (`/tmp/eh-media-staging.tgz` або `PUSH_MEDIA_STAGING`), потім окремим `railway ssh` викликається **`tar xzf`** на volume — без одного довгого `bash -c` (Railway CLI псує такі рядки). Показує кількість файлів і розмір; під час передачі — `pv` (`brew install pv`) або heartbeat + `tar -v`. `PUSH_MEDIA_QUIET=1` — без цього.
