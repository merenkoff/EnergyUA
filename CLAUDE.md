...
- The routes are `/`, `/catalog`, `/catalog/[slug]` (with `?tag=a,b` label filter), `/tag/[slug]`, `/product/[slug]`, `/brands`, `/brand/[slug]` and `/calc` (area calculator: `src/lib/heatingCalc.ts` picks mats by `area_m2` and cables by `power_w` / `length_m` laying step; GET form `CalcForm`, also on the home page). Every page is server-rendered and queries Prisma directly through the singleton in `src/lib/prisma.ts`.
...
- **preDeploy:** `npm run db:predeploy:railway`, which runs migrations, the seed and the price-list catalog import. If `RAILWAY_REBUILD_CATALOG=yes`, it then also wipes the imported catalog and re-imports it from `data/scrape/*.json` (`scripts/railway-catalog-rebuild.sh`).
